    ...
    return (
        <div className={"h-[38px] fixed top-0 left-0 w-full px-3 mb-1 pt-1"}>
            <div className={"flex justify-between items-center h-[100%]"}>
                <span className={"flex font-bold text-xs content-center flex-wrap"}>{date.getHours()}:{String(date.getMinutes()).padStart(2, "0")}</span>

                {
                    currentUser && (
                        <div className={"h-full flex align-middle"}>
                            <Link to={"/system/notifications"}
                                  className={`${location.pathname.includes("system/notifications")
                                      ? "text-primary-200" : "text-white"}`}><MdNotifications
                                className={"flex h-[38px] mr-3"} style={{"fontSize": "20px"}}/></Link>
                            <div className={"flex content-center flex-wrap"}><Space/></div>
                            <div className={"text-xs flex content-center flex-wrap"}>
                                <span onClick={() => store.dispatch(selectUser(null))}
                                      className={"text-xs flex content-center flex-wrap font-bold"}>{currentUser?.username}</span>
                            </div>
                        </div>
                    )
                }
